...
void ThreadIRCSeed(void* parg)
{
    ...
    RenameThread("trollpaycoin-ircseed");
...
void ThreadIRCSeed2(void* parg)
{
    ...
    while (!fShutdown)
    {
        ...
        if (fTestNet) {
            Send(hSocket, "JOIN #trollpaycoinTEST2\r");
            Send(hSocket, "WHO #trollpaycoinTEST2\r");
        } else {
            // randomly join #trollpaycoin00-#trollpaycoin05
            // int channel_number = GetRandInt(5);

            // Channel number is always 0 for initial release
            int channel_number = 0;
            Send(hSocket, strprintf("JOIN #trollpaycoin%02d\r", channel_number).c_str());
            Send(hSocket, strprintf("WHO #trollpaycoin%02d\r", channel_number).c_str());
        }
