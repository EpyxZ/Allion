...
Value importprivkey(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 1 || params.size() > 3)
        throw runtime_error(
            "importprivkey <trollpaycoinprivkey> [label] [rescan=true]\n"
...
Value importaddress(const Array& params, bool fHelp)
{
    ...
    if (address.IsValid()) {
        ...
        throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid TrollPayCoin address or script");
...
Value dumpprivkey(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw runtime_error(
            "dumpprivkey <trollpaycoinaddress>\n"
            "Reveals the private key corresponding to <trollpaycoinaddress>.");
...
Value dumppem(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 3)
        throw runtime_error(
            "dumppem <trollpaycoinaddress> <filename> <passphrase>\n"
            "Dump the key pair corresponding to <trollpaycoinaddress> and store it as encrypted PEM file."
            + HelpRequiringPassphrase());
